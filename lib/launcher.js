...
async function launch() {
  ...
  async function load(window) {
    ...
    const info = JSON.parse(await window.evaluate(() => document.title));
    window.load(`${appName}.html?${paramString}`, rpc.handle(new Backend(window, info)));
  }
}
